...
async def process_document(db: AsyncSession, document_id: uuid.UUID, tenant_id: uuid.UUID):
    ...
    Each step updates document.status so the frontend can poll for progress.
    """
    ...
    try:
        ...
        # Step 1: Textract OCR
        raw_response = analyze_expense(doc.s3_key)
        ...
        # Step 2: Validation (Bedrock + algorithmic)
        algo_results = _algorithmic_validation(structured)
        bedrock_results = validate_invoice_fields(structured)
